...
MODEL_OUT = "models/linear_rating.pkl"
...
def load_data(path=DATASET, min_contests=10):
    df = pd.read_csv(path)
    # drop users with fewer than min_contests rows to reduce noise from sparse features
...
def train(X_train, y_train):
    # impute nulls with column median, then scale — both required for linear regression
    pipeline = Pipeline([
...
def plot_coefficients(pipeline, feature_names, top_n=20):
    ...
    # show top_n by absolute magnitude
    ...
    plt.title(f"Top {top_n} Features by Magnitude — Linear Regression Rating Predictor")
    plt.tight_layout()
    plt.savefig("feature_importance_linear.png", dpi=150)
    plt.show()
    print("Saved feature_importance_linear.png")
